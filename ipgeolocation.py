...
"""
   ...
   vaporwave - Retrieve IP Geolocation information 
...
import sys, os
from core.vaporwaveLib import vaporwaveLib
...
def main():
    ...
    if(args.uagent and args.ulist):
        logger.PrintError("You can either specify a user-agent string or let vaporwave pick random user-agent strings for you from a file.", args.nolog)
        sys.exit(6)
        
    #specify proxy or random
    if(args.proxy and args.xlist):
        logger.PrintError("You can either specify a proxy or let vaporwave pick random proxy connections for you from a file.", args.nolog)
        sys.exit(7)
        
        
    #init lib
    ipGeoLocRequest = vaporwaveLib(args.target, logger, args.noprint)
