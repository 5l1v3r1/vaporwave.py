...
"""
    This file is part of vaporwave tool.
    Copyright (C) 2015-2016 @maldevel
    https://github.com/maldevel/vaporwave
...
from core.vaporwave import vaporwave
...
class vaporwaveLib:
    ...
    def GetInfo(self, userAgent, targetsFile=None, 
        ...
        try:
            ...
            if googleMaps and type(results) is vaporwave:
    ...
    def __exportResultsToCSV(self, objToExport, csvFile):
        ...
        if type(objToExport) is vaporwave:
    ...
    def __retrieveGeolocation(self, target):
        ...
        if response.code == 200:
            ...
            ipGeoLocObj = vaporwave(query, json.loads(response.read().decode(encoding)))
            
            self.Logger.Print('Geolocation information has been retrieved for {}({}).'.format(query, ipGeoLocObj.IP))
            
            if not self.NoPrint:
                self.Logger.Printvaporwave(ipGeoLocObj)
